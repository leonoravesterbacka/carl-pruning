...
def draw_unweighted_distributions(x0, x1, weights, variables, vlabels, binning, legend, do, n, save = False):
    ...
    for id, column in enumerate(columns, 1):
        ...
        plt.yscale('log')
        plt.hist(x0[:,column], bins = binning[id-1], weights=weights, label = "p0(x)", **hist_settings0)
        plt.hist(x1[:,column], bins = binning[id-1], label = "p1(x)", **hist_settings1)
        ...
        if save:
            ...
            plt.savefig("plots/%s_nominalVsVar_%s.png"%(id, n))                                                                
            plt.clf()
            plt.close()
...
def draw_weighted_distributions(x0, x1, weights, variables, vlabels, binning, label, legend, do, n, save = False):
    ...
    for id, column in enumerate(columns, 1):
        ...
        plt.yscale('log')
        plt.hist(x0[:,column], bins = binning[id-1], label = "p0(x)", **hist_settings0)
        plt.hist(x0[:,column], bins = binning[id-1], weights=weights, label = 'p0(x)*CARL', **hist_settings0)
        plt.hist(x1[:,column], bins = binning[id-1], label = "p1(x)", **hist_settings1)
        plt.xlabel('ttbar %s, %s'%(do, vlabels[id-1]), horizontalalignment='right',x=1) 
        ...
        if save:
            create_missing_folders(["plots"])                                                              
            plt.savefig("plots/w_%s_nominalVsVar_%s.png"%(id,label, n)) 
            plt.clf()
            plt.close()
